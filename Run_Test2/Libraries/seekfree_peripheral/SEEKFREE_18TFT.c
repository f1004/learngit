...
//-------------------------------------------------------------------------------------------------------------------
void lcd_displayimage7725(uint16 *p, uint16 width, uint16 height)
{
    int i,j; 
...
/*
void lcd_show_ZZF_image(uint8 BinaryImage[ROW][COL] )
{
  for(int i=0;i<ROW;i++)//100
  {
    if(i%2==0)
    {
      for(int j=0;j<COL;j++)//150
      {
        if(j%2==0)
        {
          if( BinaryImage[i][j]>120)
          {

            lcd_drawpoint(j/2,64-i/2,BLACK);
          }
          else
          {lcd_drawpoint(j/2,64-i/2,WHITE);
