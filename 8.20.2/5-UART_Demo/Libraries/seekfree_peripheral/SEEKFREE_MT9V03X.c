...
int16 MT9V03X_CFG[CONFIG_FINISH][2]=
{
    ...
    {EXP_TIME,          100}, //曝光时间          摄像头收到后会自动计算出最大曝光时间，如果设置过大则设置为计算出来的最大曝光值.Warning 经过实践发现每次曝光时间都会到达最大值
...
/********************************
             全局二值化
********************************/
uint8 Image_Array[ROW][COL];
uint8 threshold;
void Image_Binary(uint8 Image[ROW][COL], uint8 Image_Array[ROW][COL], int GateValue)
{
    for(int i = 0; i < ROW; i++)
    {
      for(int j = 0; j < COL; j++)
