...
extern uint8 Image_Array[ROW][COL];
...
extern unsigned char adapt_otsuThreshold(uint8 *image, uint16 col, uint16 row,unsigned char *threshold) ;
extern void halve_image(unsigned char *p_in,unsigned char  *p_out,unsigned char row,unsigned char col); //图像减半
